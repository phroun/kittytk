	...
	protocol.RegisterType("mdipane", &protocol.TypeSpec{
		...
		Bind: func(ctx *protocol.BindContext, target any) {
			...
			m.SetOnActiveWindowChanged(func(w *window.Window) {
				ev := protocol.NewEvent("active").WithUint("widget", id)
				if w != nil {
		...
		Append: func(parent, child any) error {
			...
			case core.Widget:
				...
				return nil
			default:
				return fmt.Errorf("mdipane: children must be windows or a content widget, got %T", child)
			}
		},
		Destroy: func(t any) error {
			return destroyWidget(t.(*MDIPane))
		},
	})
}
