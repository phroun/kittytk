...
func init() {
	protocol.RegisterType("scrollarea", &protocol.TypeSpec{
		...
		Props: map[string]protocol.PropertyApplier{
			...
			"resizable": boolProp("resizable", (*ScrollArea).SetWidgetResizable),
		},
		Append: func(parent, child any) error {
			...
			if !ok {
				return fmt.Errorf("scrollarea: content must be a widget, got %T", child)
			}
			if sa.Content() != nil {
				return fmt.Errorf("scrollarea: only one content widget (wrap several in a panel)")
			}
			sa.SetContent(w)
			return nil
		},
		Destroy: func(t any) error {
			return destroyWidget(t.(*ScrollArea))
		},
	})
}
