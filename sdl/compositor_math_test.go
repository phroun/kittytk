...
// The rasterized shadow: sized to the quad, opaque-ish under the
// caster, fading outward through the blur margin, and unioned with the
// anchor so the gap between control and dropdown is covered.
func TestShadowImage(t *testing.T) {
	caster := core.UnitRect{X: 100, Y: 100, Width: 60, Height: 40}
	spec := shadowSpec{offsetX: 2, offsetY: 3, blur: 8, radius: 0, alpha: 0.5}

	img := shadowImage(caster, core.UnitRect{}, spec, 1, 1)
	if img == nil {
		t.Fatal("no shadow image")
	}
	quad := shadowQuadBounds(caster, core.UnitRect{}, spec)
	if got := img.Bounds(); got.Dx() != int(quad.Width) || got.Dy() != int(quad.Height) {
		t.Fatalf("image %dx%d, want %dx%d", got.Dx(), got.Dy(), quad.Width, quad.Height)
	}

	alphaAt := func(ux, uy core.Unit) int {
		x := int(ux - quad.X)
		y := int(uy - quad.Y)
		return int(img.Pix[img.PixOffset(x, y)+3])
	}

	// Center of the cast caster is at (or near) peak alpha.
	center := alphaAt(caster.X+spec.offsetX+caster.Width/2, caster.Y+spec.offsetY+caster.Height/2)
	if center < 100 { // 0.5 alpha ≈ 127
		t.Errorf("shadow center alpha = %d, want near 127", center)
	}
	// The extreme corner of the quad is essentially clear.
	if corner := alphaAt(quad.X, quad.Y); corner > 20 {
		t.Errorf("shadow corner alpha = %d, want near 0", corner)
	}
	// Shadow is cast DOWN-RIGHT: below the caster is darker than above.
	above := alphaAt(caster.X+caster.Width/2, caster.Y-3)
	below := alphaAt(caster.X+caster.Width/2, caster.Y+caster.Height+3)
	if below <= above {
		t.Errorf("below=%d not darker than above=%d — shadow is not cast downward", below, above)
	}

	// With an anchor above the caster, the gap between them is shadowed
	// too: control and dropdown cast one shape.
	anchor := core.UnitRect{X: 100, Y: 80, Width: 30, Height: 20}
	img2 := shadowImage(caster, anchor, spec, 1, 1)
	quad2 := shadowQuadBounds(caster, anchor, spec)
...
// A shadow with no blur still rasterizes (the box-blur radius rounds to
// zero) rather than dividing by zero or returning an empty image.
func TestShadowImageNoBlur(t *testing.T) {
	img := shadowImage(core.UnitRect{X: 0, Y: 0, Width: 20, Height: 10},
		core.UnitRect{}, shadowSpec{radius: 0, alpha: 1}, 1, 1)
	if img == nil {
		t.Fatal("no image for an unblurred shadow")
	}
	if a := img.Pix[img.PixOffset(10, 5)+3]; a != 255 {
		t.Errorf("unblurred shadow center alpha = %d, want 255", a)
	}
}
